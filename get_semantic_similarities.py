...
parser.add_argument('--generation_model', type=str, default='opt-350m') # TODO: change to 1.3B only? Different tokenizers?
...
generation_tokenizer = AutoTokenizer.from_pretrained(f"facebook/opt-350m", use_fast=False, cache_dir=config.data_dir) # TODO: change to 1.3B only? Different tokenizers?

tokenizer = AutoTokenizer.from_pretrained("microsoft/deberta-large-mnli")
model = AutoModelForSequenceClassification.from_pretrained("microsoft/deberta-large-mnli").cuda() # TODO: .cuda() or .to(device)?
    ...
    if len(unique_generated_texts) > 1:
        ...
        for i, reference_answer in enumerate(unique_generated_texts):
            for j in range(i + 1, len(unique_generated_texts)):
                ...
                prediction = model(torch.tensor(torch.tensor([encoded_input]), device='cuda'))['logits'] # TODO: change device
                predicted_label = torch.argmax(prediction, dim=1)

                reverse_input = qa_2 + ' [SEP] ' + qa_1
                encoded_reverse_input = tokenizer.encode(reverse_input, padding=True)
                reverse_prediction = model(torch.tensor(torch.tensor([encoded_reverse_input]), device='cuda'))['logits'] # TODO: change device
